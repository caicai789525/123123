...
int main(int argc, char** argv) {
  // 初始化 tRPC 客户端框架
  if (trpc::Client::Init(argv[0]) != 0) {
    std::cerr << "客户端框架初始化失败" << std::endl;
    return -1;
  }

  // 获取一个服务代理（Stub）用于发起RPC调用
  auto proxy = trpc::GetProxy<trpc::exp::FileTransferServiceProxy>("trpc.exp.FileTransfer");
  ...
  // 4. 发送结束信号
  if (!stream_writer->WriteDone()) {
      std::cerr << "发送结束信号失败" << std::endl;
      return -1;
  }
